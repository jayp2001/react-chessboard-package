...
import * as SelectableSparePiecesStories from './stories/basic-examples/SelectableSparePieces.stories';
...
- [Selectable spare pieces](#selectable-spare-pieces)
- [Empty board with spare pieces](#empty-board-with-spare-pieces)
...
### Selectable spare pieces

This example extends the spare pieces pattern by adding click-to-select behaviour. Clicking a spare piece selects it (shown with a highlighted border), and then clicking any square on the board places that piece. The piece stays selected so you can place multiple copies — click it again or click a different piece to change selection. Drag-and-drop still works alongside selection.

This utilises the new `isSelected`, `onSelect`, and `selectedStyle` props on the `SparePiece` component.

<Canvas of={SelectableSparePiecesStories.SelectableSparePieces} />

### Empty board with spare pieces

Same selection behaviour as above, but starting from an empty board — useful for building positions from scratch.

<Canvas of={SelectableSparePiecesStories.EmptyBoardWithSparePieces} />
